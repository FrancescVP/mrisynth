"""Inference script for the Conditional Wavelet Diffusion Model (cWDM).

Loads a trained cWDMModel checkpoint, runs DDIM denoising on full volumes,
...
def parse_args():
    ...
    p.add_argument("--checkpoint",      required=True, help="cWDM checkpoint (.pth).")
    p.add_argument("--out_dir",         default="predictions/cwdm")
...
def main():
    ...
    model.netcWDM.load_state_dict(state)
    model.netcWDM.to(device).float().eval()
