"""Train a Rectified Flow latent diffusion model for MRI synthesis.

Trains a DiffusionModelUNet in MAISI VAE latent space.  Pre-computed
latents are required (generate_latents.py).

Quick start — use a named task (auto-sets target/conditioning):
-------------------------------------------------------------
...
---------------
  ...
  t1n_t2f_to_t1c   T1w + T2FLAIR → T1CE  (best result)
  t1n_t2w_to_t1c   T1w + T2w → T1CE

Manual target/conditioning (task-free):
---------------------------------------
  uv run python scripts/train_rflow.py \\
      --latent_dir     latents/dataset500/train \\
      --val_latent_dir latents/dataset500/val \\
      --target_key t1c --cond_keys t1n t2f \\
      --n_cond 2 \\
      --name rflow_custom \\
      --device cuda:0

Validation logs latent L1 always; image-space MAE/SSIM when --vae_ckpt is given.
"""
...
def run_validation(
    ...
    for i, batch in enumerate(val_loader):
        ...
        model.forward()
        pred_lat = model.pred_latent  # (B, 4, D', H', W')
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train RFlow latent diffusion for MRI synthesis.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    g = p.add_argument_group("task (optional shortcut)")
    g.add_argument(
        "--task", default=None, choices=list(TASKS),
        help="Named synthesis task — auto-sets --target_key, --cond_keys, --n_cond.",
    )

    g = p.add_argument_group("data")
    g.add_argument("--latent_root", default=None,
                   help="Root containing <root>/train and <root>/val subdirectories.")
    g.add_argument("--latent_dir", default=None,
                   help="Training latent directory (overrides --latent_root/train).")
    g.add_argument("--val_latent_dir", default=None,
                   help="Validation latent directory (overrides --latent_root/val).")
    g.add_argument("--vae_ckpt", default=None,
                   help="MAISI VAE checkpoint for image-space val metrics (optional).")
    g.add_argument("--target_key", default="t1c",
                   help="Modality key for synthesis target (overridden by --task).")
    ...
    g.add_argument("--name",           default=None,
                   help="Experiment name (defaults to --task or 'rflow').")
...
def _resolve_task(args: argparse.Namespace) -> tuple[str, list[str], int]:
    """Return (target_key, cond_keys, n_cond) — from --task or explicit args."""
...
def main():
    ...
    name = args.name or args.task or "rflow"
    args.name = name
    device = torch.device(args.device)
    ...
    if args.vae_ckpt and Path(args.vae_ckpt).exists():
        ...
        print(f"[warn] VAE checkpoint not found: {args.vae_ckpt} — skipping image-space val")
    ...
    print(f"  Image-space val: {'yes (VAE loaded)' if vae else 'no (latent-space only)'}")
    print("=" * 60)
    ...
    print(f"  Checkpoints : {Path(args.checkpoints_dir) / name}")
